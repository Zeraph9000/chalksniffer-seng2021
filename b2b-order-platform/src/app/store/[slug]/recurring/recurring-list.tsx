...
export function RecurringList({ storeId, storeName }: { storeId: string; storeName: string }) {
  const [rows, setRows] = useState<RecurringRow[] | null>(null);

  useEffect(() => {
    fetch("/api/orders/recurring")
      .then((r) => (r.ok ? r.json() : []))
      .then((data: RecurringRow[]) => setRows(Array.isArray(data) ? data.filter((r) => r.storeId === storeId) : []))
      .catch(() => setRows([]));
  }, [storeId]);
  ...
  return (
    <section>
      <h1 className="text-2xl font-bold mb-4">Recurring orders at {storeName}</h1>
      {rows.length === 0 ? (
          ...
          {rows.map((r) => (
            <li key={r.id} className="border rounded p-3 flex justify-between items-center">
              <div>
                ...
                {r.itemSummary && <div className="text-sm text-gray-600">{r.itemSummary}</div>}
              </div>
              <button
                type="button"
                onClick={() => cancel(r.id)}
