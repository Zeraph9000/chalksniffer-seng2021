...
export function StoreHeader({ shop }: StoreHeaderProps) {
  const logoSrc = shop.logoUrl ? transformedImageUrl(shop.logoUrl, "logo") : null;
  const bannerSrc = shop.bannerUrl ? transformedImageUrl(shop.bannerUrl, "banner") : null;
    ...
    <>
      {bannerSrc && (
        ...
        <img
          src={bannerSrc}
          alt=""
          className="w-full h-[220px] object-cover border-b border-line bg-brand-soft"
        />
      )}
      <section className="border-b border-line bg-paper px-6 py-11 flex items-center gap-14 flex-wrap">
        <div className="flex items-center gap-6 min-w-0 flex-1">
          <div className="w-20 h-20 rounded-[16px] bg-brand-surface text-brand-contrast grid place-items-center font-display font-bold text-[36px] tracking-[-.025em] shrink-0 overflow-hidden">
            {logoSrc ? (
              // eslint-disable-next-line @next/next/no-img-element
              <img src={logoSrc} alt={shop.name} className="w-full h-full object-cover" />
            ) : (
              shop.monogram
            )}
          </div>
